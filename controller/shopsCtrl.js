...
class orderShops {
    ...
    async getUnpaidTotalByShop(req, res) {
        try {
            ...
            response.success(res, "To'lanmagan buyurtmalar umumiy narxi", { total: total[0]?.total || 0 });
        } catch (err) {
            response.serverError(res, err.message);
        }
    }
    ...
    async getUnpaidTotal(req, res) {
        try {
            ...
            response.success(res, "Barcha to'lanmagan buyurtmalar umumiy narxi", { total: total[0]?.total || 0 });
        } catch (err) {
            response.serverError(res, err.message);
        }
    }
    ...
    async deleteOrder(req, res) {
        try {
            const deletedOrder = await Orderlist.findByIdAndDelete(req.params.id).lean();
